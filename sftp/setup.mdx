...
|------|-----------|-------------|
| `inventory.csv` | You → nocnoc | Stock and price updates |
| `tracking.csv` | You → nocnoc | Tracking info for shipped orders |
| `orders.csv` | nocnoc → you | New orders from the platform |
| `cancelled_orders.csv` | nocnoc → you | Cancelled orders |

<CardGroup cols={2}>
  <Card title="inventory.csv" icon="download" href="/sftp/templates/inventory.csv">
    Stock and price template
  </Card>
  <Card title="tracking.csv" icon="download" href="/sftp/templates/tracking.csv">
    Tracking info template
  </Card>
  <Card title="orders.csv" icon="download" href="/sftp/templates/orders.csv">
    Orders template (nocnoc → you)
  </Card>
  <Card title="cancelled_orders.csv" icon="download" href="/sftp/templates/cancelled_orders.csv">
    Cancellations template (nocnoc → you)
  </Card>
</CardGroup>
